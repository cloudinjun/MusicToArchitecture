...
def fig5(demo: dict) -> None:
    ...
    # Carved footprints, on whichever levels this run carved. The archetype decides
    # both which level is cut and whether anything is cut at all, so the level index
    # is read from the lattice rather than assumed to be 1.
    carved = L.get("carved") or {}
    carved_levels = sorted(carved, key=int)
    for level_key in carved_levels:
        for x0_, y0_, x1_, y1_ in carved[level_key]:
            body += rect(X(x0_), Y(y1_), (x1_ - x0_) * s, (y1_ - y0_) * s,
                         "#dcdcdc", "#555", 0.8, "4 3")
    # keys
    ...
    # 6: carved region -- omitted, with its caption clause, when nothing was carved
    if carved_levels:
        c0 = carved[carved_levels[0]][0]
...
def preset_directory(run_id: str) -> Path:
    """Where the five presentation looks of *this* run were rendered.

    The figure used to point at `model_v3` whatever the rest of the set showed, so
    figure 9 could quietly exhibit a different building from figures 2-8. It now
    prefers the demo run's own presets and falls back to the stable demo only when
    that run has none.
    """
    own = ROOT / "artifacts" / "render_presets" / run_id
    return own if (own / "photoreal" / "hero.png").is_file() else (
        ROOT / "artifacts" / "render_presets" / "model_v3")


def fig9(run_id: str) -> None:
    ...
    presets = preset_directory(run_id)
    W = 1600
    x0, gapx = 30, 14
    tw = (W - 2 * x0 - gapx * 4) / 5
    th = tw * 900 / 1400
    ...
    for i, (n, lab) in enumerate(zip(names, labels)):
        x = x0 + i * (tw + gapx)
        href, asp = thumb(presets / n / "hero.png", f"preset_{n}", 560)
...
def captions(demo: dict, sel_meta: dict, tot: dict, plan_id: str, section_id: str) -> None:
    ...
    # What the archetype carved, if anything, and on which level. The old wording named
    # the theater's auditorium and stage; a library or a museum carves elsewhere or not
    # at all, and a caption that keeps saying "auditorium" would be describing a
    # different building from the one in the figure.
    presets_of = preset_directory(run)
    preset_source = (f"this run's own model, `{run}`"
                     if presets_of.name == run
                     else f"a stable saved model (`artifacts/v3_demo/model_v3.blend`)")

    carved = L.get("carved") or {}
    carved_levels = sorted(carved, key=int)
    if carved_levels:
        where = ", ".join(f"L{int(k):02d}" for k in carved_levels)
        carved_plan_clause = (f"the region the {a['typology']} archetype carved on "
                              f"{where} is tinted")
        carved_key_clause = f"; 6 plates removed on {where}"
    else:
        carved_plan_clause = "this run carved no plate"
        carved_key_clause = ""
...
with height; the terrace plate is dashed; {carved_plan_clause}. Keys: 1 primary bay, x (`bay_x_m` {dat['bay_x_m']['value']:.2f} m, snapped to {L['plan_x_m'] / n_bays_x:.2f} m); 2 primary bay, y
...
(`cantilever_m` {dat['cantilever_m']['value']:.2f} m){carved_key_clause}. (b) Level table:
...
**Figure 9.** One model — {preset_source} — rendered through five presentation-only
