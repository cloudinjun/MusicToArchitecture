...
- Current model evidence is promoted only by
  `backend.scripts.publish_model_version`. `artifacts/model_versions/latest.json` must
  point to one immutable archive whose hashes match `artifacts/model_versions/latest/`.
  Current screenshots use `latest/renders`; archived screenshots cite the full version
  ID and status. Never cite an unlabelled model-specific render folder as current. The
  model and run identities include the exact compiler-source fingerprint; generation
  aborts if that source changes while the run is executing.
- Rhino owns issued drawings and the Revit handoff. A `.3dm` enters `latest/` only with
  an `accepted_geometry` sidecar matching its hash, run ID, and model ID. Blender `.blend`
  and GLB files remain presentation-only even when they are the newest render assets.
...
sheet is what those readings are issued on. `drawings.py` cuts, `drawing_standard.py`
answers every weight, `drawing_sheet.py` puts the result on paper. Decision
`docs/decisions/0018-the-issued-set.md` records the rules; the ones that bite:

- **One paper size per set, chosen by the largest drawing.** Drawings of a kind are
  composed onto sheets in issue order by `_pack`; two sections stack, small plans
  share a row. Sheet numbers are A-1nn plans, A-2nn elevations, A-3nn sections,
  A-000 the cover. The SVG on disk and the URL in the payload are named by the sheet
  number, and a sheet row carries its `drawings` -- do not assume one drawing per file.
- **Nothing on a sheet is authored.** Title block, key plan, captions and the cover's
  facts and 1:400 miniatures are read off the model and the lattice. No date, no
  author: two issues of one model must be byte-identical.
- **Every stroke still comes from the standard.** A hatch is a fill pattern, not a
  new weight; a caption rule is `Weight.MEDIUM`. The sheet test asserts every
  `stroke-width` on a composed sheet is one of the ISO series.
- **The cut applies the judgement a person used to apply by eye,** and each rule is a
  named constant or function: figures are `entourage` and never cut; the lift shaft
  is cut hollow by `_shaft_geometry` (a drawing fact, not a compiler one); anything
  narrower than `THIN_LINE_PAPER_MM` beyond the cut is drawn as its axis; the earth is
  hatched; the clip edge is never stroked; `resolve_section_offset` steps a section
  off any wall it would lie inside and the caption says so; `NEVER_OVERHEAD_KINDS`
  keeps railings from being dashed over a plan. Add a rule here, with its reason,
  before adding a special case in an emitter.
- **An elevation is a section whose plane is outside the building.** Nothing is cut;
  a cutaway run's open faces are drawn as open and captioned as such. With elevations
  the `on_no_cut` bucket can reach zero, and the account still has to sum.
- The set is regenerated by `python -m backend.scripts.generate_web_demo`; the
  promoted web sheets live at `web/public/drawings/latest/A-*.svg`, and their immutable
  source version is named by `artifacts/model_versions/latest.json`.
...
- **Every payload field is referenced somewhere in the client, and a script proves
  it.** The review that found `materials`, `spatial`, `archetype` and
  `tectonic_system` unmounted did so by diffing `GenerationResponse` /
  `AnalysisBundle` field names against every `web/` source: rerun that diff after any
  backend field lands (it is four lines of Python; see the session notes in this
  file's history) and mount what it names before the change is called done. The
  spatial rules and sightlines live in Compliance, the material palette in Structure,
  the tectonic family on the Frame panel's subtitle.
- **Toolbar words are the user's words.** “Labels”, “Info”, “Play” — not “Notes”,
  “HUD”, “Build”. The stage hint tells the viewer which state they are in (a cut, a
  layer filter, an assembly) and how to leave it; labels step aside while the model is
  being orbited and return when the pointer lifts.
...
`portfolio/` at the repository root is the single home for every source image meant for
a portfolio, a paper, or a company reviewer: architecture and pipeline diagrams, specimen
grids, drawing reproductions, the verification table. Before drawing any such image
anywhere else, read `portfolio/README.md`.

- Replace or add inside `portfolio/`; never start a second figure set under
  `artifacts/`, `docs/`, or `web/`.
- Every figure is authored as an SVG by `backend/scripts/render_paper_figures.py` and
  rasterised from that same SVG. Edit the generator, not the PNG.
- Figures follow the paper-figure contract stated in `portfolio/README.md`: no title,
  subtitle, data strip or stamp on the canvas; grayscale chrome; Times; real thumbnails
  and drawn data read from run records; every claim and number lives in
  `portfolio/captions.md`, never in the image.
- `portfolio/figures_manifest.json` pins the SHA-256 of every SVG and PNG and names the
